 ...
 */
const fileSize2Bytes = (fileSize, thousand) => {
  if (!fileSize) return null
  const baseUnit = thousand ? 1000 : 1024
  const kb = baseUnit; const mb = kb * baseUnit; const gb = mb * baseUnit
  const tmpCode = fileSize.substring(fileSize.length - 2).toUpperCase()
  const sizeNumber = fileSize.substring(0, fileSize.length - 2)
  const num = Number.parseInt(sizeNumber)
  let result = 0
  switch (tmpCode) {
    case 'KB':
      result = num * kb
