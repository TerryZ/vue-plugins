...
export default {
  ...
  methods: {
    paginationJson () {
      ...
      const size = this.pageSizeNum === -1 ? 10 : this.pageSizeNum
      const start = (num - 1) * size; const end = (num * size) - 1
      let list = this.data.concat()
      // do sort
      if (this.sorting && this.sortCol && list[0].hasOwnProperty(this.sortCol)) {
        const order = this.sortOrd === con.orderAsc || this.sortOrd === con.orderDesc ? this.sortOrd : con.orderAsc
    ...
    paginationServer () {
      ...
      if (this.dataLoad && typeof this.dataLoad === 'function') { this.dataLoad(this, this.data, params).then(values => { this.dataRender(values) }) }
    }
  }
}
