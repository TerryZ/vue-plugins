<template>
  <dropdown ref="drop" :border="false" @show-change="showChange">
    <template #caller>
    ...
    <div class="rg-column-container">
      <v-column
        @click.native="operLevel=con.PROVINCE_LEVEL"
        v-if="listProvince.length"
        :list="listProvince"
        :have-child="city"
        v-model="dProvince"
      ></v-column>
      <v-column
        @click.native="operLevel=con.CITY_LEVEL"
        v-if="listCity.length"
        :list="listCity"
        :have-child="area"
        v-model="dCity"
      ></v-column>
      <v-column
        @click.native="operLevel=con.AREA_LEVEL"
        v-if="listArea.length"
        :list="listArea"
        :have-child="town && haveTown"
        v-model="dArea"
      ></v-column>
      <v-column
        @click.native="operLevel=con.TOWN_LEVEL"
        v-if="listTown.length"
        :list="listTown"
        :have-child="false"
        v-model="dTown"
      ></v-column>
